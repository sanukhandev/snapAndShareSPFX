...
//             images={post.images}
//             initialIndex={this.state.selectedImage}
//           />
//         )}
//       </div>
//     );
...
interface PostProps {
  ...
  likedPosts: Set<number>;
  likedComments: Set<number>;
  onLikePost: (postId: number, updatedLikes: string) => void;
  onAddComment: (postId: number, comment: string) => void;
}
...
class Post extends React.Component<PostProps, PostState> {
  constructor(props: PostProps) {
    super(props);
    const initialLikes = props.post.likes ? props.post.likes.split(";") : [];
    this.state = {
      isModalOpen: false,
      selectedImage: 0,
      newComment: "",
      likes: initialLikes,
      isLiked: initialLikes.includes(props.userId.toString()),
    };
  }
      ...
      <div className="post-container border p-4 mb-5 rounded-3">
        ...
        <div className="row">
          {post.images.slice(0, 2).map((img, index) => (
            <div key={index} className="col-4">
              <div
                className="w-100 position-relative rounded-4"
                onClick={() => this.setState({ selectedImage: index, isModalOpen: true })}
              >
                <img
                  className="img-thumbnail w-100 rounded-4 p-0"
                  src={img}
        ...
        <div className="d-flex align-items-center my-4">
          ...
          >
            <FontAwesomeIcon
              icon={faHeart}
              className={this.state.isLiked ? "text-danger" : "text-gray-500"}
            />
            <span className="ms-2">{this.state.likes.length}</span>
          </button>
        </div>
